...
for session in sessions:
    for task_name in selected_tasks:
        ...
        ffmin = input("Enter the min frequency for spectral connectivity analysis: ").strip()
        ffmax = input("Enter the max frequency for spectral connectivity analysis: ").strip()
        ...
        epochs_block = [list(range(i*18, (i+1)*18)) for i in range(9)]
        ...
        baseline_con = spectral_connectivity_epochs(
                ...
                fmax=ffmax,
                faverage=True,
                sfreq=baseline_epochs.info['sfreq'],
                verbose=False
                )
        
        baseline_con_corrected = []

        for block in baseline_epochs.get_data():
            block = block[np.newaxis, :, :]  # Add a new axis to make it 3D
            ...
            baseline_con_corrected.append(con)
            
        
        # baseline_path = os.path.join(save_dir, f"{base_filename}_iCoh_{ffmin}_{ffmax}_baseline.fif")
        # baseline_con.save(baseline_path)

        # dense_baseline_con = baseline_con.get_data(output='dense')[:, :, 0]
        # print("Shape of dense connectivity matrix: ", dense_baseline_con.shape)
        ##Here you need to make the baseline icoh absolute value --> Shiu-Cheng
        dense_baseline_con = [bsc.get_data(output='dense')[:, :, 0] for bsc in baseline_con_corrected]
        dense_baseline_con = np.abs(np.array(dense_baseline_con))
        print("✅ Done: Shape of baseline connectivity matrix (abs): ", dense_baseline_con.shape)
        ...
        for i in range(9):  # Loop over each block 0 to 8
            ...
            dense_task_con = np.abs(dense_task_con)
            ...
            baseline_for_block = dense_baseline_con[i, :, :] # < -- if you want CORRECTION WITH FIRST BASELINE FOR ALL BLOCKS, replace i with 0
            # print("Shape of baseline for block: ", baseline_for_block.shape) # 61, 61
            # print("Shape of dense task connectivity matrix: ", dense_task_con.shape) # 61, 61
            dense_con = dense_task_con - baseline_for_block 
            # check if there is any negative value
            # if np.any(dense_con < 0):
            #     print("Negative values found in the corrected connectivity matrix.")
            # else:
            #     print("No negative values found in the corrected connectivity matrix.")
            ...
            dense_con_sym = dense_con + dense_con.T - np.diag(dense_con.diagonal())
            # dense_con_lower = np.tril(dense_con_sym, k=-1)
            
            # all_con.append(dense_con) #Append the list with the dense connectivity matrix for each block
            all_con.append(dense_con_sym) #Append the list with the dense connectivity matrix for each block
            
            df = pd.DataFrame({
                "Ch1": [channel_names[i] for i in range(61) for j in range(61)],
                "Ch2": [channel_names[j] for i in range(61) for j in range(61)],
                "Value": dense_con_sym.flatten()
                })

            pivot_df = df.pivot(index="Ch1", columns="Ch2", values="Value").fillna(0)
            
            # print(f"df ch1: {df['Ch1']}")
            # print(f"df ch2: {df['Ch2']}")
            # print(f"df value: {df['Value']}")
            ...
            mask = np.triu(np.ones_like(pivot_ordered, dtype=bool), k=0)  # Mask upper triangle and diagonal
...
for idx, dense_con in enumerate(all_con):
    ...
    pivot_ordered = pivot_df.loc[ordered_channels, ordered_channels]
    # mask = pivot_ordered.values == 0
    # mask = np.triu(dense_con_sym, k=0)
    mask = np.triu(np.ones_like(pivot_ordered, dtype=bool), k=0)
