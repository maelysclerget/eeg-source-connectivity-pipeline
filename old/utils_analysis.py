...
from mne.time_frequency import EpochsTFRArray, tfr_multitaper, tfr_morlet
...
def tfr_timewarp(tfr, durations, resample=None):
    ...
    for i, epoch in enumerate(tfr.data):
        ...
        cropped = epoch[..., np.arange(start[i], stop[i]) + int(baseline_idx.sum())]
    ...
    return EpochsTFRArray(tfr.info, data, times, tfr.freqs)


def tfr_timewarp_multichannel(
    epochs, durations, freqs, n_cycles, time_bandwidth,resample=None, average=True, n_jobs=1
):
    ...
    for i in range(0, len(chs), n_jobs):
        ...
        tfr.crop(tmin=tfr.times[0], tmax=tfr.times[-1])
...
def tfr_timewarp_multichannel_morlet(
    epochs, durations, freqs, n_cycles,resample=None, average=True, n_jobs=4
):
    """Compute time-warped TFRs in parallel.
