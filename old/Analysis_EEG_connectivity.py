...
for session in sessions:
    for task_name in selected_tasks:
        ...
        epochs = mne.read_epochs(fif_path, preload=True) #Remember here you have the epochs for all the blocks together
        ...
        #Compute the connectivity for low beta
        all_con = []
        con_values = []
        ...
        for i in range(9):  # Loop over each block 0 to 8
            print(f"Computing iCoh for epochs in block {i}...")
             ...
             # === Get baseline (first epoch of the block) === #
            baseline_epoch = epochs[[epochs_block[0][0]]].get_data(picks="eeg") 
            # [i] if you want to use the baseline before each block and [0] if you want to use the first epoch of the first block
            print(baseline_epoch.shape)
            ...
            baseline_path = os.path.join(save_dir, f"{base_filename}_iCoh_{ffmin}_{ffmax}_baseline_block_{i}.fif")
            baseline_con.save(baseline_path)

            dense_baseline_con = baseline_con.get_data(output='dense')[:, :, 0]
            
             # === Get task epochs (remaining 9) === #
            task_epochs = epochs[epochs_block[i][1:]].get_data(picks="eeg")
            ...
            #Baseline correction using the first epoch of the block which is the baseline and ramp-up
            dense_con = dense_task_con - dense_baseline_con
            ...
            fig.colorbar(im)
            plt.title("Dense connectivity matrix")
            plt.xlabel("Channels")
            plt.ylabel("Channels")
            ...
            con_values.append([i, min_val, max_val, mean_val])
            ...
            # ---- Step 1: Build region-ordered channel list ---- #
            ordered_channels = []
            region_boundaries = []
            region_names = []
        ...
        df_con_values = pd.DataFrame(con_values, columns=["Block", "Min", "Max", "Mean"])
